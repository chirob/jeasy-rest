...
public class EchoResourceTest extends AuthCoreTest {
    ...
    public void test5() throws IOException {
        IChannel channel = IResourceFinder.INSTANCE.find("/test/services/echo").getChannel(Method.GET);
    ...
    public void test6() throws IOException {
        IChannel channel = IResourceFinder.INSTANCE.find("/test/services/echo").getChannel("get");
        Source target = new Source(channel, "utf8");
        new Source("Hello, Resource!").writeTo(target);
        target.writeTo(out);
    }
    ...
    public void test7() throws IOException {
        IChannel target = IResourceFinder.INSTANCE.find("/test/services/echo").getChannel(Method.GET);
        new Source("Hello, Resource!").writeTo(new Source(target, "utf8"));
        new Source(target).writeTo(out);
    }

    public void test8() throws IOException {
        IChannel target = IResourceFinder.INSTANCE.find("/test/services/echo").getChannel("get");
        new Source("Hello, Resource!").writeTo(new Source(target, "utf8"));
        new Source(target).writeTo(out);
    }
}